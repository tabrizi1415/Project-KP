...
<main>
	<div>
		...
		<div>
			<h2 style="margin-left: 200px; margin-top: 100px;">Buat Soal - Ujian Tengah Semester - <?= $ambilmapel['mapel']; ?></h2>
		</div>
	</div>
		...
		<div class="card p-4">
			<form id="soalForm" action="" method="post">
				<div id="soalContainer">
					...
					$getdata = mysqli_query($mysqli, "SELECT * FROM ujian, mapel where ujian.id_mapel=mapel.id_mapel and mapel.id_mapel='$getmapel' and ujian.jenis_ujian='Ujian Tengah Semester';");
					...
					<input type="text" name="jenis_ujian" value="Ujian Tengah Semester" hidden>
					<!-- Soal will be appended here -->
				</div>
				<div class="text-end">
					<button type="button" class="btn btn-secondary me-2" id="tambahSoalBtn">Tambah Soal</button>
					<button type="submit" class="btn btn-primary" name="addSoalUTS">Kirim</button>
				</div>
			</form>
		</div>
	</div>
